...
class order(models.Model):
	...
	customer = models.ForeignKey('account.customer', verbose_name=_('order cutomer'), help_text='customer who placed this order', on_delete=models.SET_NULL, null=True, blank=True)
	...
	# shipping info
	shippping = models.BooleanField(_('shipping'), default=False)
	shipping_address = models.ForeignKey('account.address', null=True, blank=True, verbose_name=_('shipping address'), on_delete=models.SET_NULL)

	# payment info
	amount_charged = models.BigIntegerField(_('amount charged'), help_text=('amount for charged for order'))
	pre_tax_amount = models.BigIntegerField(_('pre tax amount'), help_text='order total before tax')
	tax_amount = models.BigIntegerField(_('taxes charged'), help_text='amount charged for taxes')
	shipping_amount = models.BigIntegerField(_('shipping charged'), help_text='amount charged for shipping')
	...
	# discounts info
	coupons_used = models.ManyToManyField('discount.coupon', verbose_name=_('coupons used on this order'))
...
class wish_list(models.Model):
	customer = models.ForeignKey('account.customer', verbose_name=_('customer'), on_delete=models.CASCADE)
	...
	products = models.ManyToManyField('product.configured_product')
